...
async def test_loyalty_id_sensor(hass: HomeAssistant) -> None:
    """Test loyalty id sensor customer profile attributes."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        title="Lidl Store 123",
        data={
    ...
    mock_data = {
        "offers": [],
        "preview_offers": [],
        "loyalty_id": "12345678901234567",
        "user_profile": {
            "user_name": "John Doe",
            "email": "user@example.com",
            "country": "DE",
            "registration_date": "2020-10-17",
        },
    }
    ...
    with patch(
        ...
        state = hass.states.get("sensor.lidl_plus_account_de_loyalty_card_id")
        assert state is not None
        assert state.state == "12345678901234567"
        assert state.attributes["user_name"] == "John Doe"
        assert state.attributes["email"] == "user@example.com"
        assert state.attributes["country"] == "DE"
        assert state.attributes["registration_date"] == "2020-10-17"
